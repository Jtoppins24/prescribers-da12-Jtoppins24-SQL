-- In this set of exercises you are going to explore additional ways to group and 
--organize the output of a query when using postgres. 

-- For the first few exercises, we are going to compare the total number of claims from 
--Interventional Pain Management Specialists compared to those from Pain Managment specialists.
...
(SELECT 'combined sum', SUM(prescription.total_claim_count)
FROM prescriber
JOIN prescription
USING(npi)
	WHERE prescriber.specialty_description = 'Interventional Pain Management'
...
SELECT prescriber.specialty_description AS specialty, SUM(prescription.total_claim_count)
FROM prescriber
JOIN prescription 
USING(npi)
	WHERE prescriber.specialty_description = 'Interventional Pain Management'
	OR prescriber.specialty_description = 'Pain Management'
GROUP BY GROUPING SETS(specialty)
;

-- 4. In addition to comparing the total number of prescriptions by specialty, 
--let's also bring in information about the number of opioid vs. non-opioid claims by these two specialties. 
--Modify your query (still making use of GROUPING SETS so that your output also shows the total number 
...
-- 8. In this question, your goal is to create a pivot table showing for each of the 4 largest cities in 
--Tennessee (Nashville, Memphis, Knoxville, and Chattanooga), the total claim count for each of six common types of opioids: 
--Hydrocodone, Oxycodone, Oxymorphone, Morphine, Codeine, and Fentanyl. 
--For the purpose of this question, we will put a drug into one of the six listed categories if it has the category name 
--as part of its generic name. For example, we could count both of "ACETAMINOPHEN WITH CODEINE" and "CODEINE SULFATE" 
